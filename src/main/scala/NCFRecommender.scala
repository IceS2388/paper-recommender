...
class NCFRecommender(ap: NCFParams) extends Recommender {
  ...
  override def train(trainingData: TrainingData): Unit = {
    ...
    val computationGraphConf = new NeuralNetConfiguration.Builder()
      ...
      .graphBuilder()
      .addInputs("input")
      .addLayer("GML", new DenseLayer.Builder().activation(Activation.RELU).nIn(2*ap.k).nOut(1).build(), "input")
      .addLayer("MLP4", new DenseLayer.Builder().activation(Activation.RELU).nIn(2*ap.k).nOut(4 * (2*ap.k)).build(), "input")
