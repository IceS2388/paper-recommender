...
class NCFClusterRecommender(ap: NCFClusterParams) extends Recommender {
  ...
  override def train(data: TrainingData): Unit = {
    ...
    val computationGraphConf = new NeuralNetConfiguration.Builder()
      ...
      .graphBuilder()
      .addInputs("input")
      .addLayer("GML", new DenseLayer.Builder().nIn(unit).nOut(2).build(), "input")
      .addLayer("MLP4", new DenseLayer.Builder().nIn(unit).nOut(4*unit).build(), "input")
      ...
      .addLayer("out", new OutputLayer.Builder().nIn(unit+2).nOut(2).build(), "ncf")
      .setOutputs("out")
      .build()
